...
use crate::db;
use crate::schema::cluster_usage_month_table;
use diesel::prelude::*;
use serde::{Deserialize, Serialize};
use chrono::{NaiveDateTime, Utc};

#[derive(Debug, Serialize, Deserialize, Queryable, QueryableByName)]
#[table_name="cluster_usage_month_table"]
pub struct ClusterUsageMonth {
    /*
    +---------------+---------------------+------+-----+---------+-------+
...
impl ClusterUsageMonth {
    pub fn find_all() -> Result<Vec<Self>, ApiError> {
        let conn = db::connection()?;

        let a = cluster_usage_month_table::table
            .load::<ClusterUsageMonth>(&conn)?;
    ...
    pub fn findById(id_tres: i32) -> Result<Self, ApiError> {
        ...
        let a = cluster_usage_month_table::table
            .filter(cluster_usage_month_table::id_tres.eq(id_tres))
            .first(&conn)?;
    ...
    pub fn findByTimeStarts(time_start: u64) -> Result<Self, ApiError> {
        ...
        let a = cluster_usage_month_table::table
            .filter(cluster_usage_month_table::time_start.eq(time_start))
            .first(&conn)?;
