...
use crate::db;
use crate::schema::cluster_usage_day_table;
use diesel::prelude::*;
use serde::{Deserialize, Serialize};
use chrono::{NaiveDateTime, Utc};

#[derive(Debug, Serialize, Deserialize, Queryable, QueryableByName)]
#[table_name="cluster_usage_day_table"]
pub struct ClusterUsageDay {
    /*
    +---------------+---------------------+------+-----+---------+-------+
...
impl ClusterUsageDay {
    pub fn find_all() -> Result<Vec<Self>, ApiError> {
        let conn = db::connection()?;

        let a = cluster_usage_day_table::table
            .load::<ClusterUsageDay>(&conn)?;
    ...
    pub fn findById(id_tres: i32) -> Result<Self, ApiError> {
        ...
        let a = cluster_usage_day_table::table
            .filter(cluster_usage_day_table::id_tres.eq(id_tres))
            .first(&conn)?;
    ...
    pub fn findByTimeStarts(time_start: u64) -> Result<Self, ApiError> {
        ...
        let a = cluster_usage_day_table::table
            .filter(cluster_usage_day_table::time_start.eq(time_start))
            .first(&conn)?;
